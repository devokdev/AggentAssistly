...
import prj3bot.channels.matrix as matrix_module
from prj3bot.bus.events import OutboundMessage
from prj3bot.bus.queue import MessageBus
from prj3bot.channels.matrix import (
    MATRIX_HTML_FORMAT,
    TYPING_NOTICE_TIMEOUT_MS,
    MatrixChannel,
)
from prj3bot.config.schema import MatrixConfig
...
async def test_start_skips_load_store_when_device_id_missing(
    ...
    monkeypatch.setattr("prj3bot.channels.matrix.get_data_dir", lambda: tmp_path)
    monkeypatch.setattr(
        "prj3bot.channels.matrix.AsyncClientConfig",
        lambda **kwargs: SimpleNamespace(**kwargs),
    )
    monkeypatch.setattr("prj3bot.channels.matrix.AsyncClient", _fake_client)
    monkeypatch.setattr(
        "prj3bot.channels.matrix.asyncio.create_task", _fake_create_task
    )
